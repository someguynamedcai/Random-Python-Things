...
print ("The following locations are: Elk Grove, Rancho Cordova, Citrus Heights, Natomas, Galt, and East Sacramento. \nPlease wait while the auction information is being loaded.")
...
#SItems = {"auction_id": S_Id, "filters[perpage]": Sjson['auctions'][Sauctions]['item_count'] }
EItems = {"auction_id": E_Id, "filters[perpage]": Ejson['auctions'][Eauctions]['item_count']}
RItems = {"auction_id": R_Id, "filters[perpage]": Rjson['auctions'][Rauctions]['item_count']}
CItems = {"auction_id": C_Id, "filters[perpage]": Cjson['auctions'][Cauctions]['item_count']}
NItems = {"auction_id": N_Id, "filters[perpage]": Njson['auctions'][Nauctions]['item_count']}
GItems = {"auction_id": G_Id, "filters[perpage]": Gjson['auctions'][Gauctions]['item_count']}
ESItems = {"auction_id": ES_Id, "filters[perpage]": ESjson['auctions'][ESauctions]['item_count']}
            ...
            if number == "exit" or number == "Exit":
                ...
                match location:       
                    case 'E':
                        ...
                        Fullauctionlink = auctionlink + Ejson['auctions'][Eauctions]['id'] + "/item/" + EPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                    ...
                    case 'R':
                        ...
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + Rjson['auctions'][Rauctions]['id'] + "/item/" + RPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
                    ...
                    case 'C':
                        ...
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + Cjson['auctions'][Cauctions]['id'] + "/item/" + CPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
                    ...
                    case 'N':
                        ...
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + Njson['auctions'][Nauctions]['id'] + "/item/" + NPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
                    ...
                    case 'G':
                        ...
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + Gjson['auctions'][Gauctions]['id'] + "/item/" + GPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
                    ...
                    case 'ES':
                        ...
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + ESjson['auctions'][ESauctions]['id'] + "/item/" + ESPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
                    ...
                    case 'RO':
                        print ("Item number " + number + " is titled " + ROPost['items'][int(number)-1]['title'])
                        print ("The current price of item number " + number + " is $" + ROPost['items'][int(number)-1]['current_bid'])
                        print ("The total price calculated with the 8.25% tax and 13% buyer's premium is $" + str(float(ROPost['items'][0]['current_bid']) * .0825 + float(ROPost['items'][0]['current_bid']) + float(ROPost['items'][0]['current_bid']) * .13))
                        cleanedq = Cleanquery.sub("", ROPost['items'][int(number)-1]['title'])
                        Fulllink = cccprice + str(cleanedq)
                        Fullauctionlink = auctionlink + ROjson['auctions'][ROauctions]['id'] + "/item/" + ROPost['items'][int(number)-1]['id']                                                                     
                        print ("A camelcamelcamel page and a page to the item will open in 2 seconds. Item may not be related to item in gallery.")
                        time.sleep(2)
                        webbrowser.get('edge').open(Fulllink)
                        webbrowser.get('edge').open(Fullauctionlink)
...
while Redirectto != "No":
    '''if (Redirectto == "Sacramento" or Redirectto == "sacramento" or Redirectto == 's' or Redirectto == 'S'):
        print ("The first auction gallery in Sacramento is a " + (Sjson['auctions'][Sauctions]['title']))
        print ("There are " + Sjson['auctions'][Sauctions]['item_count'] + " items in this gallery")
        Itemcheck("S")
    '''
    if (Redirectto == "Exit" or Redirectto == "exit"):
        ...
        print ("There are " + Ejson['auctions'][Eauctions]['item_count'] + " items in this gallery.")
        Itemcheck("E")
        ...
        print ("The first auction gallery in Rancho Cordova is a " + (Rjson['auctions'][Rauctions]['title']))
        print ("There are " + Rjson['auctions'][Rauctions]['item_count'] + " items in this gallery.")
        Itemcheck("R")
        ...
        print ("The first auction gallery in Citrus Heights is a " + (Cjson['auctions'][Cauctions]['title']))
        print ("There are " + Cjson['auctions'][Cauctions]['item_count'] + " items in this gallery.")
        Itemcheck("C")
        ...
        print ("There are " + Njson['auctions'][Nauctions]['item_count'] + " items in this gallery.")
        Itemcheck("N")
               
    elif (Redirectto == "Galt" or Redirectto == "galt" or Redirectto == 'g' or Redirectto == 'G'):
        print ("The first auction gallery in Galt is a " + (Gjson['auctions'][Gauctions]['title']))
        print ("There are " + Gjson['auctions'][Gauctions]['item_count'] + " items in this gallery.")
        Itemcheck("G")
        ...
        print ("There are " + ESjson['auctions'][ESauctions]['item_count'] + " items in this gallery.")
        Itemcheck("ES")
        ...
        print ("There are " + ROjson['auctions'][ROauctoins]['item_count'] + " items in this gallery.")
    else:
